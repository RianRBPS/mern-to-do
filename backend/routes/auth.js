...
router.post('/register', async (req, res) => { // async allows me to use await to handle asynchronous operations like saving to database
    ...
    try {
        const user = await User.create({ name, password}); // tries to create a new user in the mongodb database using the User model, the await keyword tells node.js to pause here until the create operation finishes
